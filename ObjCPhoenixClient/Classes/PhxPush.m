...
@property (nonatomic, assign, readwrite) NSTimeInterval afterInterval;
...
- (PhxPush*)after:(NSTimeInterval)time callback:(After)callback {
    if (self.afterHook) {
        // ERROR
    }
    self.afterTimer = [NSTimer scheduledTimerWithTimeInterval:time target:self selector:@selector(afterTimerFire:) userInfo:nil repeats:NO];
    self.afterInterval = time;
